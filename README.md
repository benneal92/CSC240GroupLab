...
1 - (364 &frasl; 365)<sup>n(n-1)/2</sup>
...
The simulateBirthdayParadox function within the Driver simulates the probability of there being a collision of birthdays in a hash table. The function uses a BirthDate and the HashTable classes our team has developed. To effectively simulate the Birthday Paradox, the BirthDate object does not call the actual hash function - instead the day in the year of the birthday (from Jan. 1st) is returned to emulate people having their birthdays on the same day. The simulateBrithdayParadox function gets a number of people from the user and then outputs the **actual** and **simulated** probability (randomly generating **n** BirthDate objects, putting these birthdays in a hashtable, and running 10000 times).
...
To make these objects more personable, a random name is generated with each birthday in the default constructor. The age of the person is also calculated as well.
